...
public class QueryBuilder {
    ...
    public String buildDeleteQuery(Object object) throws IllegalAccessException {
        ...
        if (clazz.isAnnotationPresent(Table.class)) {
            StringBuilder sqlRequest = new StringBuilder("DELETE FROM ");
            Table tableAnnotation = clazz.getAnnotation(Table.class);
            sqlRequest.append(tableAnnotation.name()).append(" WHERE ");
            Field[] fields = clazz.getDeclaredFields();
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(Column.class)){
                    ...
                    if (columnAnnotation.primaryKey()) {
                        field.setAccessible(true);
                        sqlRequest.append(columnAnnotation.name()).append(" = '").append(field.get(object)).append("'");
                    }
                }
            }
